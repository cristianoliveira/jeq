...
`jeq reduce` accepts one bounded JSON collection and sends exactly one TypeSafe request. JSON input must be an array; NDJSON values are collected in input order into an array. The request state is that exact collection and the output is one `{items,_jeq}` envelope.

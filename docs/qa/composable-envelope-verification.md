...
| Nested duplicate keys rejected at top level and in nested objects (`/payload` resolution) and inside `_jeq` (existing evidence duplicate) | **PASS** |
| Trailing JSON values rejected | **PASS** |
| Invalid UTF-8 rejected (`JEQ_REQUEST_INVALID` from `scanDoc`) | **PASS** |
...
| Non-object envelope root (`[]`) rejected as `JEQ_INPUT_INVALID` | **PASS** |
| `_jeq` wrong type (array) rejected, `_jeq.<name>` collision rejected | **PASS** |
...
| Chained enrichment preserves prior `_jeq.<first>` evidence AND original arbitrary members | **PASS** |
...
| Auth (`JEQ_AUTH_REJECTED`) and interruption (`context.Canceled`) errors forwarded unchanged, sentinel equality preserved | **PASS** |
...
| Probe | Result | Calls |
| --- | --- | --- |
| `{"k":"\xff"}` (invalid UTF-8) | `JEQ_REQUEST_INVALID` (from `scanDoc`) | 0 |
| `{"a":1}{"b":2}` (trailing) | `JEQ_INPUT_INVALID` | 0 |
| `{"k":"a\x00b"}` (raw NUL) | `JEQ_INPUT_INVALID` (`invalid character`) | 0 |
| `{"k":"a\tb"}` (raw tab) | `JEQ_INPUT_INVALID` (`invalid character`) | 0 |
...
| `{"k":9007199254740993}` (huge int) | `JEQ_REQUEST_INVALID` (state must be string/object/array) | 0 |
| `{"a":[1,2]}` + pointer `/a/-1` (negative index) | `JEQ_INPUT_INVALID` | 0 |
| `{"x":[1]}` + pointer `/x/01` (noncanonical) | `JEQ_INPUT_INVALID` (already covered by test) | 0 |
...
| --- | --- | --- | --- |
| `""` | `{"payload":{"x":1},"unicode":"café"}` | byte-exact record | full record + `_jeq.step` |
| `/payload` | `{"payload":{"x":1}}` | `{"x":1}` | original + `_jeq` |
| `/a~1b/~0key` | `{"a/b":{"~key":"value"}}` | `"value"` | original + `_jeq` |
| `/items/1` | `{"items":["first",{"second":true}]}` | `{"second":true}` | original + `_jeq` |
...
assert members["_jeq"]["first"] != nil
assert members["_jeq"]["second"] != nil
```
...
- Sees an existing `_jeq` object (with `first` already in it).
- Adds `second` to that `_jeq` object (collision check passes; `first` ≠ `second`).
...
| "`_jeq` is absent or an object" | `existingEvidence` decodes via `decodeObject`; non-object `_jeq` fails (`existing_evidence_wrong_type`) | matches |
| "Existing `_jeq.<name>` is never overwritten and is a local input error" | Explicit `_, exists := jeqMembers[config.Name]; exists` check (`collision`) | matches |
...
| "Complete typed response: typed answers, model, usage, unknown response fields" | `response.Encode()` + `server_extra` round-trip; `TestPointersPreserveRawStateAndCallOnce` checks `server_extra` after `decodeOutput(members["_jeq"]["step"])` | matches |
...
| "Local envelope/pointer errors use `JEQ_INPUT_INVALID` before evaluator construction" | All `TestEnvelopeCollisionsNamesAndDuplicateKeys` subtests + `TestNamesAndChainedEnrichment` name subtests | matches |

...
Status: independent QA verification of the v1 release wave — local deterministic probes plus paid production runs through the compiled `jeq` binary. No code or plans changed; nothing committed.
...
| Validate source-conflict failure → structured exit 2 (`JEQ_SOURCE_CONFLICT`) | **PASS** |
| Validate offline **with TYPESAFE_API_KEY removed**: exit 0, no env lookup, no network | **PASS** |
| Models without key → `JEQ_AUTH_MISSING`, exit 1, no client created | **PASS** |
| Models bad `--base-url` → `JEQ_INPUT_INVALID`, exit 2 | **PASS** |
| Models bad `--timeout` → `JEQ_INPUT_INVALID`, exit 2 | **PASS** |
| Paid `jeq models` against production: 200, parseable aliases, 1 line | **PASS** |
| Paid `jeq ask` default output, composed mode, pinned `jev-1.13.0`: valid JSON, exit 0 | **PASS** |
...
- `--output toon` rejection (binary probe, streams separated):
  ...
  ```
  $ /tmp/jeq-d3 ask --questions /tmp/q.json --state "x" --output toon >o.txt 2>e.txt
  exit=2  stderr=[]  stdout: valid-json
  code=JEQ_INPUT_INVALID  recovery=set --output json
  ```

- Code inspection (`internal/cli/discovery.go::outputRenderer`): the validator returns `JEQ_INPUT_INVALID` with recovery `"set --output json"` for any value other than `json`. There is no code path that can produce TOON bytes; the renderer has only one implementation (`internal/infra/render/json.go`).

### Home view (`jeq` with no args)

```
$ unset TYPESAFE_API_KEY TYPESAFE_BASE_URL TYPESAFE_DEFAULT_MODEL
$ /tmp/jeq-d3 >o.txt 2>e.txt
exit=0  stdout_lines=1  stderr=[]  stdout: valid-json
stdout=[{"identity":"jeq","purpose":"agent-first TypeSafe System One client","credential_ready":false,"default_model":"jev-latest","commands":["ask","models","validate","version","help"],"next_step":"run jeq ask --help"}]
...
```
$ /tmp/jeq-d3 version >o.txt 2>e.txt
exit=0  stdout_lines=1  stderr=[]  stdout: valid-json
name,version,commit = jeq,dev,unknown
```
...
| conflict: `--request a --questions b --state x` | — | **2** | structured `JEQ_SOURCE_CONFLICT` | empty | n/a |
...
```
unset TYPESAFE_API_KEY → exit 1, code=JEQ_AUTH_MISSING  (recovery: export TYPESAFE_API_KEY)
TYPESAFE_API_KEY=fake models --base-url "not-a-url" → exit 2, code=JEQ_INPUT_INVALID
TYPESAFE_API_KEY=fake models --timeout nope → exit 2, code=JEQ_INPUT_INVALID
```
...
### Paid production — `jeq models` and default-output `jeq ask`
...
| --- | --- | --- | --- | --- | --- |
| `jeq models` (real) | 626 ms | 0 | empty | one parseable JSON line, 2 aliases | `models: [jev-latest, jev-preview]` |
| `jeq ask --questions q.json --state-file - --model jev-1.13.0` (default output) | 663 ms | 0 | empty | one parseable JSON line, **no `--output` flag used** | resolved `jev-1.13.0`; 391 in / 73 out tokens; choice sum = 1.0; score sum = 1.0; confidence in [0,1] |
...
`--state-json` accepts a file path (or `-` for piped stdin), not an inline JSON literal — consistent with the `ask` command. A first-time operator may try `--state-json '{"k":"v"}'` and the source reader treats the curly-brace string as a path, returning `JEQ_INPUT_INVALID: source … does not exist`. Worth a one-line example in `jeq validate --help` if/when help text is upgraded (current help is Cobra-generated list form). Not a code defect; no fix proposed.

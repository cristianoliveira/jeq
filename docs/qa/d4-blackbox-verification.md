...
| Check | Result |
| --- | --- |
| Black-box suite builds binary once via TestMain (`go build ./cmd/jeq`), no `cli.Run` calls | **PASS** |
...
| SIGINT on blocked request → exit 130, structured `JEQ_INTERRUPTED` doc, **stderr empty** | **PASS** |
| Connection drop → exit 1, `JEQ_NETWORK_ERROR`, **stdout contains no raw EOF/reset/key**, stderr empty | **PASS** |
| Auth (missing key / wrong key) → `JEQ_AUTH_MISSING` / `JEQ_AUTH_REJECTED`, no secret leak | **PASS** |
| Timeout → `JEQ_TIMEOUT`, single request | **PASS** |
| Malformed 200 → `JEQ_RESPONSE_INVALID`, single request | **PASS** |
...
| Usage errors → `JEQ_INPUT_INVALID` exit 2, structured recovery, no Cobra prose | **PASS** |
...
ok  github.com/cristianoliveira/jeq/internal/blackbox  (cached)
```
...
```
$ rg "fmt\.Fprintf\(os\.Stderr|DEBUG" internal/infra/typesafeapi/ internal/infra/render/ internal/infra/source/ cmd/jeq/
(no matches)
...
This is needed because Go's `net/http` transport translates a signal-driven syscall interrupt (`EINTR`) into a `*url.Error` whose message is `interrupt signal received` — it does **not** unwrap to `context.Canceled`. Without checking `ctx.Err()`, the error fell through to `JEQ_NETWORK_ERROR`. With it, the same context-canceled state correctly classifies as `JEQ_INTERRUPTED`.

### Behavioral assertions (the cases Mary asked to re-run)

| Scenario | Asserted (in test) | Observed |
| --- | --- | --- |
| **SIGINT blocked ask** | exit `130`; stdout one JSON doc with `code:"JEQ_INTERRUPTED"`; stderr empty; no secret (`interrupt-secret`) in stdout | **PASS** — test asserts all four; test passes |
| **Timeout** (`--timeout 50ms` against `<-r.Context().Done()` handler) | exit `1`; `code:"JEQ_TIMEOUT"`; **exactly 1 request** | **PASS** — server counted 1 |
| **Connection drop** (hijacker.Close) | exit `1`; `code:"JEQ_NETWORK_ERROR"`; **exactly 1 request**; stdout contains no `"EOF"`/`"connection reset"`; stderr empty; no secret (`drop-secret`) leaked | **PASS** — `api.count()==1`, no transport prose in stdout |
...
| **Malformed 200** | exit `1`; `code:"JEQ_RESPONSE_INVALID"`; **exactly 1 request** | **PASS** |
| **Low confidence** (`confidence: 0.01`) | exit `0`; full success document rendered; no secret leak | **PASS** |
| **Auth: missing key** | exit `1`; `code:"JEQ_AUTH_MISSING"`; **exactly 0 requests** | **PASS** |
| **Auth: 401** | exit `1`; `code:"JEQ_AUTH_REJECTED"`; **exactly 1 request**; secret not echoed | **PASS** |
...
| **Usage errors** (unknown command / flag / misspelling / `--output toon` on version) | exit `2`; `code:"JEQ_INPUT_INVALID"`; non-empty `recovery`; no Cobra prose on stderr | **PASS** |

...
class PolicyTests(unittest.TestCase):
 ...
 def test_matching_target_and_finite_value(self):
  r=self.p.request(self.elements,"u","t"); target=next(k for k,v in self.p._candidates.items() if v["operation"]=="fill"); d=self.p.consume({"model":"m","answers":{"operation":{"choice":"fill"},"fill":{"choice":target},"fill_value":{"choice":"value_0"}}},r); self.assertEqual((d.operation,d.ref,d.value),("fill","e2","Lisbon"))

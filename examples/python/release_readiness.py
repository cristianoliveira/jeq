...
def judgments(response: dict[str, Any]) -> tuple[dict[str, Any], float, float, str, float]:
    risk = answer(response, "risk")
    manual = answer(response, "manual_review")
    rollout = answer(response, "rollout")
    risk_score = bounded(risk, "score", "risk")
...
def policy(response: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    evidence, risk, manual, rollout, rollout_confidence = judgments(response)
    if risk >= RISK_BLOCK_MIN or manual >= RISK_BLOCK_MIN or rollout == "hold":
        return "block", "risk, manual review, or rollout policy blocks release", evidence
    if risk <= RISK_PASS_MAX and manual <= RISK_PASS_MAX and rollout == "full":
        return "pass", "low risk supports full rollout", evidence
    if risk <= RISK_CANARY_MAX and manual <= RISK_CANARY_MAX and rollout == "canary":
        return "canary", f"bounded risk supports canary rollout at confidence {rollout_confidence:.2f}", evidence
    return "review", "evidence does not satisfy pass or canary policy", evidence


def receipt(response: dict[str, Any]) -> tuple[dict[str, Any], int]:
    try:
        decision, reason, evidence = policy(response)

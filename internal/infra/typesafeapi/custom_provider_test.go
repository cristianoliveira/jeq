...
func TestCustomLoopbackProviderEvaluatesWithoutModelsPreflight(t *testing.T) {
 ...
 srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
  ...
  if r.URL.Path != "/v1/systemone" { http.NotFound(w, r); return }
 ...
 if _, err := client.Evaluate(context.Background(), sampleRequest(t)); err != nil { t.Fatalf("loopback evaluate failed: %v", err) }
 if len(paths) != 1 || paths[0] != "POST /v1/systemone" { t.Fatalf("requests=%v, want only judgment POST", paths) }
}

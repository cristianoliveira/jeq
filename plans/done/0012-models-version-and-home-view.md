...
## Acceptance criteria
- [ ] `jeq models` calls the real models client with normal auth/timeout/base-url classification and renders the server's alias list without assuming versioned IDs are listed.
- [ ] `jeq version` emits injected build name/version/commit deterministically; release values come from linker flags, development values remain explicit.

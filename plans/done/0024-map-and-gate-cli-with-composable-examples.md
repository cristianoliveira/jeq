...
## Desired outcome
`jeq map` performs one named semantic enrichment per record; `jeq gate` performs one named offline numeric policy decision. `jq` and the shell remain the branching and orchestration language.

## Acceptance criteria
- [ ] `jeq map --as <name>` accepts JSON or NDJSON, sequentially enriches each object under `_jeq.<name>`, preserves order, and emits the same framing. Exactly one mode is required: composed `--questions <file> [--state-pointer <RFC6901>] [--model]`, or native `--request-pointer <RFC6901>` for a strictly validated request already constructed in the record by ordinary tools such as `jq`.
...
- [ ] `jeq gate --as <name> --value-pointer <RFC6901> --pass-min <0..1> --reject-max <0..1>` is offline, appends pass/reject/uncertain evidence, and supports JSON/NDJSON. Aggregate exit is 10 if any reject, else 11 if any uncertain, else 0; operational/usage/interruption stay 1/2/130.

...
## Problem
Bare jeq currently prints a custom status dashboard. The root command should follow standard Cobra behavior and show help/usage instead of credential/model state.

## Acceptance criteria
- [x] Bare `jeq` prints the same standard Cobra help as `jeq --help` on stdout, exits 0, and writes no stderr.
...
- [x] Keep `jeq examples` as explicit workflow discovery and keep all TASK-0030 output boundaries unchanged.

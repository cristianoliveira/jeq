...
- [x] `jeq rate` accepts the established JSON-object and NDJSON record framing
      through `--input json|ndjson`.
- [x] `--as`, `--state-pointer`, `--instruction`, and at least two ordered
      `--level` values are required.
- [x] `--level` order is preserved exactly as the Score criteria order; blank
      levels and blank instructions fail before credential lookup or network
      access.
- [x] The command supports the same `--model`, `--config`, `--base-url`,
      `--timeout`, and `--max-retries` behavior and precedence as `map`.
- [x] Bare `jeq rate` shows native Cobra help, matching other action commands.

### Request and output

- [x] For each record, `rate` resolves state through `--state-pointer` and sends
      exactly one Score question named by `--as`, with the supplied instruction
      and ordered level strings.
- [x] `rate` makes one System One request per input record, matching `map`; its
      help and docs state this cost explicitly.
- [x] Every successful record is preserved and receives the complete lossless
...
- [x] Existing `_jeq.<as>` collisions, invalid pointers, malformed framing,
      partial failures, API failures, retries, timeouts, and interruption retain
      the established `map` behavior and exit semantics.
- [x] `rate` delegates through the existing map/request pipeline rather than
      implementing a second evaluator loop or response envelope.

### Composition and meaning

- [x] Documentation demonstrates sorting, top-k, filtering, and optional
      explicit `jeq gate`/`jq` policy over the returned score.
- [x] Documentation states that Score is an independent semantic rating against
      ordered descriptions, not an exact measurement or arithmetic result.
- [x] Documentation distinguishes `rank`/Choice (relative competition among
      candidates) from `rate`/Score (the same rubric applied independently, so
      every record can rate high or low).
- [x] `rate` adds no implicit threshold, sorting, top-k, batching, action,
      fallback, or conversion of the score into a physical quantity.

### Verification and documentation

- [x] Tests are written first and cover JSON and NDJSON success, level order,
...
- [x] A built-binary fake-endpoint test verifies stdin/stdout/stderr, exit status,
      exact request count, Score request shape, and composition through `jq`.
- [x] Root help, `jeq rate --help`, README command tables, architecture guide,

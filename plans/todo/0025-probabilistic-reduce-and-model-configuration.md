...
- [ ] `gev reduce --as <name> --questions <file> [--input json|ndjson] [--model <override>]` accepts a JSON array or bounded NDJSON values, preserves order and raw JSON semantics, and produces one JSON envelope: `{"items":[...],"_gev":{"<name>":<complete response>}}`.
